...
import { DocumentUpdate } from "@prisma/client";
...
export async function POST(
  ...
  try {
    ...
    const permission = await db.documentPermission.findUnique({
      where: {
        documentId_userId: { documentId: docId, userId: session.id as string },
      },
    });
    if (!permission || permission.role !== "OWNER") {
      return NextResponse.json(
        { error: "Forbidden: Only owners can compact logs" },
        { status: 403 }
      );
    }

    // 3. Extract all sequential binary fragments
    const updates: DocumentUpdate[] = await db.documentUpdate.findMany({
    ...
    if (updates.length <= 1) {
      return NextResponse.json({
        success: true,
        message: "Log layout already optimal.",
      });
    }
    ...
    for (const update of updates) {
      ...
      try {
        ...
        console.error(
          `🚨 Defensively skipped corrupted row reference [ID: ${update.id}]:`,
          decodeError
        );
      }
    }
    ...
    return NextResponse.json(
      { error: "Internal compaction execution fault" },
      { status: 500 }
    );
  }
}
