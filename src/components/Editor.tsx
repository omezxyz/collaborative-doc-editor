...
function TipTapCanvas({
        ...
        try {
          ...
          if (res.ok) {
            ...
            const title =
              (Array.isArray(data)
                ? data[0]?.title
                : data.title || data.document?.title) || "Untitled Document";
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-invert prose-slate max-w-none focus:outline-none min-h-[800px] p-12",
      },
    },
    onUpdate: () => {
  ...
  const handleAiMutation = async (
    promptType: "improve" | "summarize" | "fix-grammar" | "continue"
  ) => {
    ...
    if (!selectedText.trim()) {
      alert(
        "Please enter or select some text first before calling AI workflows."
      );
      setAiDropdownOpen(false);
      return;
    }
      ...
      if (response.ok) {
        ...
        if (promptType === "continue") {
          ...
          editor
            .chain()
            .focus()
            .insertContentAt({ from, to }, generatedText)
            .run();
        }
      } else {
        ...
        <div className="flex items-center gap-4 text-xs" role="status">
          ...
          >
            <RefreshCw
              size={12}
              className={
                syncState === "syncing" ? "animate-spin text-blue-400" : ""
              }
            />
        ...
        <div className="flex items-center gap-1 bg-slate-900/50 border-b border-slate-800 px-4 py-1.5 shrink-0">
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              title={
                !isOnline
                  ? "Connect to network to use AI features"
                  : "AI Actions"
              }
            >
              {aiLoading ? (
          ...
          {isOnline ? (
            ...
            <div className="flex flex-col items-center justify-center h-full p-6 text-center opacity-70 mt-24">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                Connect to the internet to manage permissions, view active
                users, and access document history.
              </p>
            </div>
          )}
        </aside>
